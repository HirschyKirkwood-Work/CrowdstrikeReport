    ...
    def process_info(
        ...
        with open(outpath, "a") as outfile:
            with open(inactive_out, "a") as inactive:
                andrew_ids = [
                    value["andrewID"] for value in self.dsp_data_dict.values()
                ]
                for hostname, andrewID in self.dsp_data_dict.items():
                    ...
                    if (
                        hostname in self.falcon_data_dict
                        and andrewID["andrewID"] in self.andrewID_dict
                    ):
                        ...
                        outfile.write(
                            f"{hostname},{andrewID['andrewID']},{last_seen[:10]},{department}\n"
                        )
                    # Write users with computer who aren't in the right SID in Falcon
                    elif (
                        hostname in self.self_managed
                        and andrewID["andrewID"] in self.andrewID_dict
                    ):
                        department = self.andrewID_dict[andrewID["andrewID"]].replace(
                            '"', ""
                        ...
                        inactive.write(
                            f"{hostname},{andrewID['andrewID']},{last_seen[:10]},{department},They Used the Self-Managed SID\n"
                        )
                    # Users who do not show up with their machine in Falcon.
                        ...
                        inactive.write(
                            f"{hostname},{andrewID['andrewID']},{department},{andrewID['last_seen']}\n"
                        )
                ...
                for andrewID, department in self.andrewID_dict.items():
                    if andrewID not in andrew_ids:
                        inactive.write(
                            f"Not A Customer or no machine on SLA,{andrewID},{department},N/A\n"
                        )
...
if __name__ == "__main__":
    falcon = FalconInfo()
    falcon.process_info(
        "dsp_users_and_machine.csv",
        "falconusersandgroups.csv",
        "falconexport.csv",
        "active_users.csv",
        "inactive_machines.csv",
        "self_managed.csv",
    )
